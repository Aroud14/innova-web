...
$total = $conexion->consultaregistro($querys->getconteocategoriaservicio($nombre));
$resul_lista = $conexion->obtenerlista($querys->getlistcategoriaservicio($nombre, $inicio, $limite));
?>
<div class="white-box">
    ...
    <table class="table color-table info-table">
        ...
        foreach ($resul_lista as $value){
            ?>
            <tr>
                <td><td><img width="90px" src="archivos/<?php echo $value->archivo ?>"></td></td>
                <td><?php echo $value->nombre ?></td>
                <td><?php echo $funciones->estatus_tipoact($value->estatus) ?></td>
                <td>
                    <?php if($datos_sis['editar'] == 1) { ?>
                        <a type="button" href="javascript:void(0)" onclick="registro_categoriaservicio(<?php echo $value->id_categoria_servicio; ?>)" class="btn btn-inline btn-sm btn-primary" data-bs-toggle="tooltip" data-bs-placement="top" title="Editar"><i class="fa fa-edit"></i></a>
                    <?php } ?>
                    <?php if($datos_sis['eliminar'] == 1) { ?>
                        <a type="button" class="btn btn-inline btn-sm btn-danger" onclick="eliminar(29,<?php echo $value->id_categoria_servicio ?>);" data-bs-toggle="tooltip" data-bs-placement="top" title="Eliminar"><i class="fa fa-trash"></i></a>
                    <?php } ?>
                </td>
            </tr>
        <?php } ?>
                        ...
                        if($datos){
                            foreach ($datos as $enlace){
                                if($enlace['active'] == false){ ?>
                                    <li class="page-item"><a  class="page-link" href="javascript:lista_categoriablog(<?php echo $enlace['numero'] ?>);"><?php echo $enlace['vista']; ?></a></li><?php
                                }else{ ?>
