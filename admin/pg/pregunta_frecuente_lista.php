...
$total = $conexion->consultaregistro($querys->getconteopreguntafrecuente($nombre));
$resul_lista = $conexion->obtenerlista($querys->getlistpreguntafrecuente($nombre, $inicio, $limite));
?>
<div class="white-box">
    ...
    <table class="table color-table info-table">
        ...
        foreach ($resul_lista as $value){
            ?>
            <tr>
                <td><?php echo substr($value->nombre,0,20) ?>...</td>
                <td><?php echo substr( $value->respuesta,0,15) ?>...</td>
                <td><?php echo $funciones->estatus_tipoact($value->estatus) ?></td>
                <td>
                    <?php if($datos_sis['editar'] == 1) { ?>
                        <a type="button" href="javascript:void(0)" onclick="registro_preguntafrecuente(<?php echo $value->id_pregunta_frecuente; ?>)" class="btn btn-inline btn-sm btn-primary" data-bs-toggle="tooltip" data-bs-placement="top" title="Editar"><i class="fa fa-edit"></i></a>
                    <?php } ?>
                    <?php if($datos_sis['eliminar'] == 1) { ?>
                        <a type="button" class="btn btn-inline btn-sm btn-danger" onclick="eliminar(30,<?php echo $value->id_pregunta_frecuente ?>);" data-bs-toggle="tooltip" data-bs-placement="top" title="Eliminar"><i class="fa fa-trash"></i></a>
                    <?php } ?>
                </td>
            </tr>
        <?php } ?>
                        ...
                        if($datos){
                            foreach ($datos as $enlace){
                                if($enlace['active'] == false){ ?>
                                    <li class="page-item"><a  class="page-link" href="javascript:lista_preguntafrecuente(<?php echo $enlace['numero'] ?>);"><?php echo $enlace['vista']; ?></a></li><?php
                                }else{ ?>
