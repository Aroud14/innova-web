<?php
    ...
    $estado = (isset($_POST['estado2']) && ($_POST['estado2'] > 0)) ? $funciones->limpia($_POST['estado2']) : "";
    $claveInegi = (isset($_POST['clave_inegi2']) && $_POST['clave_inegi2'] != '') ? $funciones->limpia($_POST['clave_inegi2']) : "";

    $resultados = $conexion->obtenerlista($querys->getListaMunicipio($nombre, $estado, $claveInegi, $inicio, $limite));
    $totalRegistros = $conexion->consultaregistro($querys->getConteoListaMunicipio($nombre, $estado, $claveInegi));
?>
                ...
                <tbody>
                    ...
                    ?>
                        <tr>
                            ...
                            <td nowrap><?php echo $conexion->consultaregistro($querys->getNombreEstado($resultado->id_estado)) ?></td>
                            ...
                            ?> 
                                    <a type="button" href="javascript:void(0)" onclick="registro_municipio(<?php echo $resultado->id_municipio; ?>)" class="btn btn-inline btn-sm btn-primary" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-original-title="Editar"><i class="fa fa-edit"></i></a>
                            <?php
                                }
                            ?>
                            ...
                            ?>
                                    <a type="button" onclick="eliminar(4,<?php echo $resultado->id_municipio ?>);" class="btn btn-inline btn-sm btn-danger" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-original-title="Eliminar"><i class="fa fa-trash"></i></a> 
                            <?php
                                }
                            ?>
                                    ...
                                    if($datos){
                                        foreach ($datos as $enlace){
                                            if($enlace['active'] == false){ ?>
                                                <li class="page-item"><a  class="page-link" href="javascript:lista_municipio(<?php echo $enlace['numero'] ?>);"><?php echo $enlace['vista']; ?></a></li><?php
                                            }else{ ?>
